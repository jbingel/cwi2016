...
class LM:
    def __init__(self,arpafile=None,start=START,end=END,unk=UNK):
        """
        ...
        if self.isBigram: # if trigram lm, read that too
    ...
    def score_ug(self,sentence):
        words = [self.get_word_id(w) for w in sentence.strip().split()]
        val = 0.0
        for i in range(len(words)):
            val += self.ug_prob(words[i])
        return val

    def score_bg(self,sentence):
        assert self.isBigram,\
                "Can't score sentence as bigram with unigram lm."
        words = [self.get_word_id(w) for w in sentence.strip().split()]
        val = 0.0
...
if __name__ == '__main__':
    lm = LM('/home/joachim/data/simplewiki.arpa')
    print(lm.unigrams)
    p = lm.score_ug('afdfskjfhdfk')
    print(p)
    p = lm.score_ug('go')
    print(p)
    p = lm.score_ug('computer')
    print(p)