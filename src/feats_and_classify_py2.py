...
class WordInContext:
    ...
    def f_prob_in_context_feats(self):
        D = {}
        #D["f_P(w|w-1)"]    = seq_prob(self.word, [self.sentence[self.index-1]]) # prob() uses freq()
        #D["f_P(w|w-2w-1)"] = seq_prob(self.word, [self.sentence[self.index-2:self.index]]) # prob() uses freq()
        #D["f_P(w|w+1)"]    = seq_prob(self.word, [self.sentence[self.index+1]]) # prob() uses freq()
        #D["f_P(w|w+1w+2)"] = seq_prob(self.word, [self.sentence[self.index+1:self.index+3]]) # prob() uses freq()
        return D
...
def main():
    ...
    all_annotations = scriptdir+"/../data/cwi_training/cwi_training_allannotations.txt"
    ...
    parser.add_argument('--pickle', '-p', help="If set, feats and labels are written out as pickles", default='../data/feats-and-labels.pickle')
    args = parser.parse_args()

    if args.pickle:
        print('Only writing features and labels out as pickle data to {}.'.format(args.train))
        features, labels, vec = collect_features(args.train)
        labels_indiv = [collect_labels(all_annotations, i) for i in range(20)]
        np.save(args.pickle, [features, labels, labels_indiv], allow_pickle=False, fix_imports=False)
        #np.save(args.pickle, [features, labels, labels_indiv], allow_pickle=False)
        sys.exit(0)
